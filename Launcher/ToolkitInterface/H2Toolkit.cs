...
namespace ToolkitLauncher.ToolkitInterface
{
    public class H2Toolkit : ToolkitBase
    {
		...
		private H2ToolLightmapFixInjector? GetInjector(LightmapArgs args)
        {
            ...
            // A recognized stock hash uses its exact config. An UNRECOGNIZED hash is assumed to be a
            // byte-patched stock tool (e.g. tool_fast_confine / experiment builds) — byte patches don't move
            // the tag_save call sites, so fall back to the known offsets for this tool TYPE rather than
            // returning null (which would drop the farm's tag_save nopfills = broken farm).
            NopFillFormat config = _calls_to_patch_md5.TryGetValue(tool_hash, out var known)
                ? known
                : (tool == ToolType.ToolFast
                    ? new NopFillFormat(0x400000, new List<uint> { 0x4ADD50, 0x4ADFF5 })
                    : new NopFillFormat(0x400000, new List<uint> { 0x4F833F, 0x4F867B }));
		...
		public override async Task BuildLightmap(string scenario, string bsp, LightmapArgs args, ICancellableProgress<int>? progress)
		{
			...
			DLLInjector? lightmapHooksInjector = null;

			bool useCustomQuality = args.QualitySetting == "custom" && Profile.IsMCC;
			// The scenery / dynamic-object bake fix works on BOTH the MCC tool_fast AND the H2V/2007
			// tool (H2Tool.exe / H2Tool_codez.exe). The injected H2ToolHooks DLL self-detects which
			// binary it landed in (by module + prologue signature) and patches the matching addresses
			// (see LightmapSceneryFix.cpp), so this only needs enabling for any Halo 2 profile with the
			// checkbox set — MCC or Vista. H2Toolkit is H2-only so the generation guard is belt-and-braces.
			bool useSceneryFix = Profile.H2LightmapSceneryFix && (Profile.IsMCC || Profile.Generation == GameGen.Halo2);
			...
			if (useCustomQuality || useSceneryFix || usePrecisionFix)
			{
				...
				// The "keep open" output mode wraps the tool in a cmd shell (cmd /c "tool & pause") and can
				// only inject AFTER polling finds the already-running child process — too late, so the hooks
				// never take effect. Close Shell and Silent both launch the tool suspended (StartProcess ->
				// CREATE_SUSPENDED) so the fix injects first. Force keepOpen -> closeShell: still a live console
				// during the bake, just without the post-run pause. (Farm workers only; the single-worker path
				// is intentionally left alone.)
				if (args.outputSetting == OutputMode.keepOpen)
				{
					Trace.WriteLine("Lightmap fix active: forcing output mode keepOpen -> closeShell (early injection needs a suspended launch).");
					args = args with { outputSetting = OutputMode.closeShell };
				}
			...
			try
			{
				if (args.instanceCount > 1 && (Profile.IsMCC || Profile.CommunityTools)) // multi instance?
				{
                    ...
                    if (injector is not null)
                        injector.DaisyChainedInjector = lightmapHooksInjector;
					...
					async Task RunInstance(int index)
					{
						...
						if (Profile.IsMCC)
						{
                            ...
                            if (config is null && lightmapHooksInjector is not null)
                            {
								Trace.WriteLine($"Configuring injector (quality patch) for worker {index}");
								config = new(lightmapHooksInjector);
                            }
						...
						{
							// H2Codez (Vista) farm workers. Inject the scenery-fix DLL into every slave so the
							// H2V occluder decompress hook applies during each worker's geometry import — the
							// single-instance path already does this, and the farm path must too or a multi-worker
							// Vista bake silently skips the fix.
							...
							outputMode: args.outputSetting,
							injectionOptions: lightmapHooksInjector is not null ? new(lightmapHooksInjector) : null,
							cancellationToken: progress.GetCancellationToken());
						}
